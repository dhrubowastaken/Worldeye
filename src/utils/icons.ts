...
export const getIconSvg = (type: string, category: string): string => {
  const color = CAT_COLORS[category] || '#ffffff';
  let svgPath = '';

  if (type === 'air') {
    // Sleek geometric plane
    ...
    // Satellite dish / orbit shape
    svgPath = `<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z" fill="${color}" filter="drop-shadow(0px 0px 5px ${color})" />
